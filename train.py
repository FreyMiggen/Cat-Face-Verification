...
def train(datadir='img_db',margin=0.2,alpha=0,f_epochs=40,u_epochs=20,pretrained_model_path=None):
    ...
    for epoch in range(f_epochs+u_epochs):
        if epoch==f_epochs[0]:
...
def Main():
    ...
    parser.add_argument('-e','--fepochs',default=40,type=int,help='number of epochs before unfreeze')
    parser.add_argument('-ue','--uepochs',default=20,type=int,help='number of epochs after unfreeze')
    parser.add_argument('-p','--pretrained_model',help='the path to the pretrained model',default='none')
    args=parser.parse_args()

    train(args.datadir,args.margin,args.alpha,args.fepochs,args.uepochs,args.pretrained_model)
